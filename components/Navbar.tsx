    ...
    <nav className="sticky top-0 z-50 w-full border-b border-slate-800 bg-secondary/95 px-4 py-3 text-slate-300 backdrop-blur md:px-6">
      <div className="container mx-auto flex items-center justify-between">
              ...
              <div className="mt-4 flex flex-col space-y-2">
                {navLinks.map((link) => (
                  <Link
                    key={link.href}
                    href={link.href}
                    className="flex items-center rounded-md p-3 transition-colors hover:bg-slate-800"
                  >
        ...
        <div className="flex items-center space-x-2 md:space-x-4">
          {/* AI Assistant - Icon only on mobile, text on desktop */}
          <Button
            variant="outline"
            className="border-slate-700 bg-slate-800/30 px-2 hover:bg-slate-800 hover:text-white md:px-4"
          >
          ...
          <div className="flex items-center space-x-1 text-slate-400">
            <Button
              variant="ghost"
              size="icon"
              className="xs:flex hidden hover:text-white"
            >
              <Sun className="h-5 w-5" />
            </Button>

            {/* Help Center - Restored */}
            <Button
              variant="ghost"
              size="icon"
              className="hidden hover:text-white sm:flex"
            >
            ...
            <div className="relative">
              ...
              <span className="absolute top-2.5 right-2.5 h-2 w-2 rounded-full border-2 border-secondary bg-red-500"></span>
            </div>
          </div>
